package com.manisoft.scraprushapp.ui.otpverification
...
class OTPVerificationActivity : AppCompatActivity() {
    ...
    private fun saveUserData(data: LoginData, isProfileNotCompleted: Boolean) {
        ...
        if (!data.is_new_user && !isProfileNotCompleted) {
            ...
            KeyValues.writeString(Constants.MOBILE_NUMBER, data.user.mobile_number.toString())
            KeyValues.writeString(Constants.DOB, data.user.dob ?: "")
    ...
    private fun signInWithPhoneAuthCredential(credential: PhoneAuthCredential) {
        ...
        firebaseAuth.signInWithCredential(credential).addOnCompleteListener(this) { task ->
            ...
            if (task.isSuccessful) {
                ...
                viewModel.login(LoginRequest(mobile_number = user?.phoneNumber?.toLong() ?: 0))
            } else {
